                    ...
                    <div class="space-y-8">
                        ...
                        <div class="space-y-4 pt-4 border-t border-gray-100">
                            ...
                            <div class="p-6 bg-gray-50 rounded-xl border border-gray-100 space-y-4">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Monto</label>
                                        <input type="number" wire:model="newDeposit.amount" step="0.01" min="0.01" placeholder="0.00" class="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none">
                                    ...
                                    <div>
                                        <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Método de Pago</label>
                                        <select wire:model="newDeposit.payment_method" class="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none">
                                            <option value="efectivo">Efectivo</option>
                                            <option value="transferencia">Transferencia</option>
                                        </select>
                                    ...
                                    <div class="md:col-span-2">
                                        <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Notas (Opcional)</label>
                                        <textarea wire:model="newDeposit.notes" rows="2" class="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none"></textarea>
                                        @error('newDeposit.notes')
                                            <p class="text-[10px] text-red-600 mt-1">{{ $message }}</p>
                                        @enderror
                                    </div>
                                </div>
                                <button wire:click="addDeposit" class="w-full bg-blue-600 text-white py-3 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-blue-700 transition-all shadow-sm">Agregar Abono</button>
                            </div>
                            @endif
                                ...
                                <table class="min-w-full divide-y divide-gray-50">
                                    ...
                                    <tbody class="divide-y divide-gray-50">
                                        @forelse($detailData['deposit_history'] ?? [] as $deposit)
                                            <tr class="hover:bg-gray-50/50 transition-colors group">
                                                <td class="px-4 py-3 text-xs font-bold text-gray-900">{{ $deposit['created_at'] }}</td>
                                                <td class="px-4 py-3 text-xs text-center font-bold text-gray-900">${{ number_format($deposit['amount'], 0, ',', '.') }}</td>
                                                <td class="px-4 py-3 text-xs text-center">
                                                    <span class="text-[9px] font-bold uppercase {{ $deposit['payment_method'] === 'efectivo' ? 'text-emerald-600 bg-emerald-50' : 'text-blue-600 bg-blue-50' }} px-2 py-0.5 rounded-full">
