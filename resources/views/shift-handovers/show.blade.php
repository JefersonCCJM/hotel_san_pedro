...
<div class="space-y-6">
    <div class="flex justify-between items-center">
        ...
        <div class="flex gap-2">
            @if($handover->status->value === 'activo')
                <span class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-100 text-emerald-700 text-sm font-bold">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
            <h3 class="font-bold text-gray-900 mb-4 uppercase text-xs tracking-wider border-b pb-2">Informacion del Turno</h3>
            <ul class="space-y-4">
                ...
                <li class="flex justify-between items-center text-sm">
                    ...
                    <span class="font-bold text-gray-900">{{ $handover->ended_at ? $handover->ended_at->format('H:i') : '—' }}</span>
                </li>
            ...
            <div class="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <p class="text-xl font-bold text-gray-900">${{ number_format($handover->base_inicial, 0, ',', '.') }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 font-medium uppercase mb-1">Ventas Efectivo</p>
                        <p class="text-xl font-bold text-emerald-600">${{ number_format($handover->total_entradas_efectivo, 0, ',', '.') }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 font-medium uppercase mb-1">Ventas Transferencia</p>
                        <p class="text-xl font-bold text-blue-600">${{ number_format($handover->total_entradas_transferencia, 0, ',', '.') }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 font-medium uppercase mb-1">Total Salidas</p>
                        <p class="text-xl font-bold text-red-600">${{ number_format($handover->total_salidas, 0, ',', '.') }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 font-medium uppercase mb-1">Base Esperada</p>
                        <p class="text-xl font-bold text-indigo-600">${{ number_format($handover->base_esperada, 0, ',', '.') }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 font-medium uppercase mb-1">Base Recibida</p>
                        <p class="text-xl font-bold text-{{ $handover->base_recibida !== null && abs($handover->diferencia) > 0 ? 'red' : 'emerald' }}-600">
                ...
                <div class="mt-6 p-4 rounded-lg bg-{{ abs($handover->diferencia) > (float)config('shifts.difference_tolerance', 0) ? 'red' : 'amber' }}-50 border border-{{ abs($handover->diferencia) > (float)config('shifts.difference_tolerance', 0) ? 'red' : 'amber' }}-100 flex items-center justify-between">
                    <div class="flex items-center gap-3 text-{{ abs($handover->diferencia) > (float)config('shifts.difference_tolerance', 0) ? 'red' : 'amber' }}-700 font-bold">
                        <i class="fas fa-exclamation-triangle"></i>
                        <span>Diferencia en caja:</span>
                    </div>
                    <span class="text-lg font-black text-{{ abs($handover->diferencia) > (float)config('shifts.difference_tolerance', 0) ? 'red' : 'amber' }}-700">
                    ...
                    <table class="min-w-full divide-y divide-gray-100">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-[10px] font-black text-gray-500 uppercase">Habitación</th>
                                <th class="px-4 py-3 text-left text-[10px] font-black text-gray-500 uppercase">Huésped</th>
                                <th class="px-4 py-3 text-left text-[10px] font-black text-gray-500 uppercase">Reserva</th>
                                <th class="px-4 py-3 text-left text-[10px] font-black text-gray-500 uppercase">Check-out</th>
                                <th class="px-4 py-3 text-left text-[10px] font-black text-gray-500 uppercase">Estado Pago</th>
                                <th class="px-4 py-3 text-right text-[10px] font-black text-gray-500 uppercase">Valor</th>
                            </tr>
                        </thead>
                            ...
                            @foreach($shiftRentals as $stay)
                                @php
                                    ...
                                    $rowTotal = isset($stay->rental_total)
                                        ...
                                        : ($subtotal > 0 ? $subtotal : (float) ($reservation->total_amount ?? 0));
                                ...
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap">
                                        <div class="flex flex-col gap-1">
                                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold border w-fit {{ $statusClass }}">
                                                {{ $statusLabel }}
                                            </span>
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap text-sm text-right font-bold text-indigo-700">
                    ...
                    <table class="min-w-full divide-y divide-gray-100">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-[10px] font-black text-gray-500 uppercase">Notas</th>
    ...
    <div class="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <p class="text-xs text-gray-500 font-medium uppercase mb-1">Entrega ({{ $handover->receptionist_display_name }})</p>
                <p class="text-sm text-gray-700 italic">{{ $handover->observaciones_entrega ?: 'Sin observaciones' }}</p>
            </div>
            <div>
                <p class="text-xs text-gray-500 font-medium uppercase mb-1">Recepcion ({{ $handover->recibidoPor->name ?? 'N/A' }})</p>
                    ...
                    <table class="min-w-full divide-y divide-gray-100">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-center text-[10px] font-black text-gray-500 uppercase">Metodo</th>
                                <th class="px-4 py-3 text-right text-[10px] font-black text-gray-500 uppercase">Efectivo</th>
                                <th class="px-4 py-3 text-right text-[10px] font-black text-gray-500 uppercase">Transf.</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-3 whitespace-nowrap text-xs text-gray-400 font-mono">#{{ $sale->id }}</td>
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-sm text-right font-bold text-emerald-600">
                    ...
                    <table class="min-w-full divide-y divide-gray-100">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-[10px] font-black text-gray-500 uppercase">Habitacion</th>
                                <th class="px-4 py-3 text-left text-[10px] font-black text-gray-500 uppercase">Huesped</th>
                                <th class="px-4 py-3 text-left text-[10px] font-black text-gray-500 uppercase">Producto</th>
                                <th class="px-4 py-3 text-center text-[10px] font-black text-gray-500 uppercase">Cant.</th>
                                <th class="px-4 py-3 text-center text-[10px] font-black text-gray-500 uppercase">Metodo</th>
                                <th class="px-4 py-3 text-center text-[10px] font-black text-gray-500 uppercase">Estado</th>
                                <th class="px-4 py-3 text-right text-[10px] font-black text-gray-500 uppercase">Total</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-100">
                            @foreach($shiftRoomSales as $roomSale)
                                ...
                                @endphp
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-3 whitespace-nowrap text-xs text-gray-400 font-mono">#{{ $roomSale->id }}</td>
                                    <td class="px-4 py-3 whitespace-nowrap text-xs text-gray-500">
                ...
                @else
                    <table class="min-w-full divide-y divide-gray-100">
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-3 whitespace-nowrap text-xs text-gray-500">{{ $outflow->created_at->format('H:i') }}</td>
                                <td class="px-4 py-3 text-sm text-gray-700">{{ Str::limit($outflow->reason, 40) }}</td>
                ...
                @else
                    <table class="min-w-full divide-y divide-gray-100">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-4 py-3 text-left text-[10px] font-black text-gray-500 uppercase">Hora</th>
                                <th class="px-4 py-3 text-left text-[10px] font-black text-gray-500 uppercase">Concepto</th>
                                <th class="px-4 py-3 text-right text-[10px] font-black text-gray-500 uppercase">Monto</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-3 whitespace-nowrap text-xs text-gray-500">{{ $cashOut->created_at->format('H:i') }}</td>
                                <td class="px-4 py-3 text-sm text-gray-700">{{ Str::limit($cashOut->concept, 40) }}</td>
            ...
            <table class="min-w-full divide-y divide-gray-100">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-[10px] font-black text-gray-500 uppercase">Motivo</th>
                        <th class="px-4 py-3 text-center text-[10px] font-black text-gray-500 uppercase">Cant.</th>
                        <th class="px-4 py-3 text-left text-[10px] font-black text-gray-500 uppercase">Observaciones</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    @foreach($handover->productOuts->sortByDesc('created_at') as $out)
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 whitespace-nowrap text-xs text-gray-500">{{ $out->created_at->format('H:i') }}</td>
                        ...
                        <td class="px-4 py-3 text-xs text-gray-500">{{ Str::limit($out->observations, 50) ?: '—' }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
