...
<div class="space-y-6">
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-amber-700">
                        Entregado por <span class="font-bold">{{ $pendingReception->receptionist_display_name }}</span>
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div class="bg-white rounded-xl border border-gray-100 p-4 shadow-sm text-center">
            ...
            <p class="text-xl font-bold text-gray-900">${{ number_format($pendingReception->base_inicial, 0, ',', '.') }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-100 p-4 shadow-sm text-center">
            <p class="text-[10px] font-black text-emerald-600 uppercase tracking-wider mb-1">Ventas Efectivo</p>
            <p class="text-xl font-bold text-emerald-600">${{ number_format($pendingReception->total_entradas_efectivo, 0, ',', '.') }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-100 p-4 shadow-sm text-center">
            <p class="text-[10px] font-black text-blue-600 uppercase tracking-wider mb-1">Ventas Transferencia</p>
            <p class="text-xl font-bold text-blue-600">${{ number_format($pendingReception->total_entradas_transferencia, 0, ',', '.') }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-100 p-4 shadow-sm text-center">
            <p class="text-[10px] font-black text-red-600 uppercase tracking-wider mb-1">Total Salidas</p>
            <p class="text-xl font-bold text-red-600">${{ number_format($pendingReception->total_salidas, 0, ',', '.') }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-100 p-4 shadow-sm text-center">
            <p class="text-[10px] font-black text-indigo-600 uppercase tracking-wider mb-1">Base Esperada</p>
            <p class="text-2xl font-black text-indigo-600">${{ number_format($pendingReception->base_esperada, 0, ',', '.') }}</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
        <h4 class="text-xs font-black text-gray-500 uppercase tracking-wider mb-2">
            <i class="fas fa-comment-alt mr-1"></i> Observaciones de {{ $pendingReception->receptionist_display_name }}
        </h4>
    ...
    @endphp
    @if(abs($diff) > 0)
    <div class="rounded-xl border p-4 flex items-start gap-3 {{ $diff < 0 ? 'bg-red-50 border-red-200 text-red-800' : 'bg-amber-50 border-amber-200 text-amber-800' }}">
        <i class="fas fa-exclamation-triangle mt-0.5"></i>
        <div>
            <p class="text-sm font-bold">
                {{ $diff < 0 ? 'Faltante' : 'Sobrante' }} reportado: ${{ number_format(abs($diff), 0, ',', '.') }}
            </p>
    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-xl border-2 border-blue-200 shadow-sm overflow-hidden">
            <div class="bg-blue-50 px-6 py-4 border-b border-blue-100">
                ...
                <p class="text-xs text-blue-700 mt-1">Cuenta el dinero físico en caja y confirma la recepción del turno.</p>
            </div>
            <div class="p-6">
                <form action="{{ route('shift-handovers.store-reception') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="base_recibida" oninput="formatNumberInput(this)" class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-blue-500 focus:ring-0 transition-all text-2xl font-black text-center" placeholder="0" required autofocus>
                    ...
                    <div>
                        ...
                        <textarea name="observaciones" rows="3" class="w-full px-4 py-2 border-2 border-gray-200 rounded-xl focus:border-blue-500 focus:ring-0 transition-all" placeholder="Escribe aquí cualquier novedad encontrada en la caja..."></textarea>
                    </div>

                    <div class="pt-4 flex gap-3">
                        <a href="{{ route('dashboard') }}" class="flex-1 text-center px-4 py-3 bg-gray-100 text-gray-700 rounded-xl text-sm font-bold hover:bg-gray-200 transition-colors">
                            Cancelar
                        </a>
                        <button type="submit" class="flex-1 bg-blue-600 text-white py-3 rounded-xl text-sm font-black hover:bg-blue-700 transition-all shadow-md">
